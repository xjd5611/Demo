...
public class ProcedureLoadAsset : ProcedureBase
{
    ...
    private void OnOpenUISuccess(object sender, GameEventArgs e)
    {
        ...
        if (UIOpenDic.ContainsKey(openUIFormSuccessEventArgs.UIForm.UIFormAssetName))
        {
            ...
            if (!UIOpenDic.ContainsValue(false))
            {
                ...
                ChangeState<ProcedureTurnStart>(m_procedureOwner);
            }
        }
    }
    #region 加载数据
    ...
    {
        m_role = new RoleData(GameEntry.Entity.EntityId(), 1) { Position = new Vector3(-4, 0.5f, 0) };
        m_monster = new MonsterData(GameEntry.Entity.EntityId(), 1001) { Position = new Vector3(4, 0.5f, 0) };

        GameEntry.DataNode.SetData(Definition.Node.RoleNode, new VarRoleData(m_role));
        GameEntry.DataNode.SetData(Definition.Node.MonsterNode, new VarMonsterData(m_monster));
    ...
    {                  
        ...
        IDataTable<DRAttackCards> attackCardsDT = GameEntry.DataTable.GetDataTable<DRAttackCards>();
        DRAttackCards[] dRAttackCards = GameEntry.DataTable.GetDataTable<DRAttackCards>().GetAllDataRows();

        IDataTable<DRDefenseCards> defenseCardsDT = GameEntry.DataTable.GetDataTable<DRDefenseCards>();
        DRDefenseCards[] dRDefenseCards = GameEntry.DataTable.GetDataTable<DRDefenseCards>().GetAllDataRows();

        IDataTable<DRSkillCards> skillCardsDT = GameEntry.DataTable.GetDataTable<DRSkillCards>();
    ...
    private void LoadCharacter()
    {
        ...
        GameEntry.Entity.CreateMonsterEntity(monsterId, m_monster);
    }
    ...
    private void LoadPlayerCards()
    {
        ...
        for (int i = 0; i < cardsId.Length; i++)
        {
            if (attackCardsDT.HasDataRow(cardsId[i]))
            {
                ...
                AttackCardData attackCardData = new AttackCardData(cardDataId, cardsId[i]);
                int cardId = GameEntry.Entity.EntityId();
